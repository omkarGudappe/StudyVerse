...
function PdfViewer({ fileUrl, className = "", from = "", id }) {
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      setScale(window.innerWidth < mobileBreakpoint ? 0.6 : 0.75);
    }
  ...
  const zoomIn = () => {
    setScale(prev => Math.min(prev + 0.10, 3.0));
  };

  const zoomOut = () => {
    setScale(prev => Math.max(prev - 0.10, 0.5));
  };
    ...
    <div>
      {documentLoaded && numPages > 1 && (
        <div className="lg:flex items-center space-x-2">
          ...
          >
            <svg className={` w-5 h-5 ${isMobile ? 'text-black' : 'text-neutral-300'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
            </svg>
          </button>

          <span className={`text-sm ${isMobile ? 'text-black' : 'text-neutral-300' }  min-w-[80px] text-center`}>
            {pageNumber} - {numPages}
          </span>
    ...
    <div>
      {documentLoaded && (
        <div className="flex items-center space-x-2">
          ...
          <span className={`text-sm ${isMobile ? 'text-black' : 'text-neutral-300' }  min-w-[60px] text-center `}>
          ...
          {/* <button
            onClick={resetZoom}
            className={`p-2 bg-neutral-700/50 ${isMobile ? 'text-black' : 'text-neutral-300'} rounded-lg hover:bg-neutral-600/50 hover:text-white transition-colors text-xs`}
          >
            Reset
          </button> */}
        </div>
  ...
  const DownloadPdf = () => (
    <button
      title="Download PDF"
      onClick={downloadFile}
      className={`${isMobile ? "bg-neutral-700/50 rounded-full p-1 " : "flex items-center space-x-2 px-3 py-2 bg-gradient-to-r from-purple-600 to-amber-500 text-white rounded-lg hover:from-purple-500 hover:to-amber-400 transition-colors text-sm"} cursor-pointer z-50`}
    >
      <svg className={` ${isMobile ? "w-8 h-8 text-blue-50" : "w-4 h-4"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className={`bg-neutral-900  overflow-hidden border border-neutral-700/50 ${className}`}>
      {/* Controls Header */}
      <div className={`${isMobile ? "hidden" : "block"} bg-neutral-800/80 backdrop-blur-sm border-b border-neutral-700/50 p-4`}>
        <div className="lg:flex items-center justify-between lg:flex-wrap gap-3">
      ...
      {from === 'feed' && isMobile && (
          ...
          to={`/pdf-notes/${id}`}
          state={{ content: fileUrl }}
          className="absolute top-7 right-10 z-10 flex items-center space-x-2 px-3 py-2 bg-neutral-800 text-white rounded-lg hover:bg-blue-500 transition-colors text-sm"
        >
          Open
        </Link>
      )}
        <div className={`${isMobile ? "w-full overflow-auto" : "flex justify-center"}  p-4`}>
          <div className={`${isMobile ? "inline-block" : "flex justify-center"}  min-w-max`}>
            {pdfBlobUrl && (
              <Document
                file={pdfBlobUrl}
      ...
      {isMobile && (
        <div className={`${from === 'feed' ? 'hidden' : 'block'}`}>
          <div className="fixed bottom-0 right-0 p-4 mb-15 text-black flex items-center z-40">
            <PageChangeComponent />
          </div>

          <div className="fixed bottom-0 left-0 p-4 mb-15 text-black flex items-center z-40">
            <ZoomControlsComponent />
          </div>

          <div className="fixed top-4 right-4 text-black z-50">
            <DownloadPdf />
          </div>
        </div>
      )}
